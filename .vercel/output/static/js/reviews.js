document.addEventListener('DOMContentLoaded', () => {
  ...
  const STAR_SVG_FILLED = '<svg class="w-5 h-5" viewBox="0 0 20 20"><path fill="' + YELLOW + '" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>';
  const STAR_SVG_EMPTY = '<svg class="w-5 h-5" viewBox="0 0 20 20"><path fill="none" stroke="' + YELLOW + '" stroke-width="1.5" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>';
  const STAR_SVG_FILLED_SM = '<svg class="w-4 h-4" viewBox="0 0 20 20"><path fill="' + YELLOW + '" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>';
  const STAR_SVG_EMPTY_SM = '<svg class="w-4 h-4" viewBox="0 0 20 20"><path fill="none" stroke="' + YELLOW + '" stroke-width="1.5" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>';
  const STAR_SVG_FILLED_INPUT = '<svg class="w-8 h-8" viewBox="0 0 20 20"><path fill="' + YELLOW + '" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>';
  const STAR_SVG_EMPTY_INPUT = '<svg class="w-8 h-8" viewBox="0 0 20 20"><path fill="none" stroke="' + YELLOW + '" stroke-width="1.5" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>';
  ...
  function initStarRatingInput() {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      btn.innerHTML = STAR_SVG_EMPTY_INPUT;
      btn.className = 'star-btn p-0.5';

      btn.addEventListener('mouseenter', () => {
        container.querySelectorAll('.star-btn').forEach((s, idx) => {
          s.innerHTML = idx < i ? STAR_SVG_FILLED_INPUT : STAR_SVG_EMPTY_INPUT;
        });
      });
    ...
    container.addEventListener('mouseleave', () => {
      container.querySelectorAll('.star-btn').forEach((s, idx) => {
        s.innerHTML = idx < selectedRating ? STAR_SVG_FILLED_INPUT : STAR_SVG_EMPTY_INPUT;
      });
    });
  }
  ...
  function renderReplyCard(reply) {
    const replyCompanyHtml = reply.company_name
      ? `<span class="text-secondary/50 dark:text-backgroundBody/50 text-xs">, ${escapeHtml(reply.company_name)}</span>`
      : '';

    return `
      <div class="reply-card p-4 mb-3">
        <div class="flex flex-wrap items-center gap-y-1 text-sm mb-2">
          <span class="font-medium">${escapeHtml(reply.author_name)}</span>
          ${replyCompanyHtml}
          <span class="text-secondary/40 dark:text-backgroundBody/40 mx-1.5">&middot;</span>
          <span class="text-secondary/40 dark:text-backgroundBody/40 text-xs">${timeAgo(reply.created_at)}</span>
        </div>
        <p class="text-sm text-secondary/80 dark:text-backgroundBody/70 leading-relaxed">${escapeHtml(reply.reply_text)}</p>
      </div>
    `;
  }
  ...
  function renderReviewCard(review) {
    ...
    const companyHtml = review.company_name
      ? `<span class="text-secondary/50 dark:text-backgroundBody/50">, ${escapeHtml(review.company_name)}</span>`
      : '';

    let repliesHtml = replies.map(renderReplyCard).join('');

    return `
      <article class="review-card p-6 md:p-8" data-review-id="${review.id}">
        <!-- Header -->
        <div class="mb-4">
        ...
        <div class="reply-thread mt-4" id="thread-${review.id}">
          <div class="reply-thread-inner ml-2 md:ml-4 pl-4 md:pl-5 border-l-2 border-secondary/15 dark:border-backgroundBody/15">
            ${repliesHtml}

            <!-- Reply Form Card -->
            <div class="reply-form-card p-4">
              <form class="reply-form" data-review-id="${review.id}">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                  <input
                    ...
                    class="reply-input py-2.5 pl-3 bg-white dark:bg-[#151515] focus:outline-none focus:border-primary border border-secondary/10 dark:border-backgroundBody/10 w-full text-colorText dark:text-backgroundBody/70 text-sm"
                  />
                  <input
                    type="text"
                    name="company_name"
                ...
                <textarea
                  ...
                  class="reply-input py-2.5 pl-3 bg-white dark:bg-[#151515] focus:outline-none focus:border-primary border border-secondary/10 dark:border-backgroundBody/10 w-full text-colorText dark:text-backgroundBody/70 text-sm resize-y mb-3"
                ></textarea>
  ...
  function bindDeleteButtons() {
    document.querySelectorAll('.delete-review-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch('/api/reviews/delete', { method: 'POST', body: formData });
  ...
  function bindReplyForms() {
    document.querySelectorAll('.reply-form').forEach(form => {
      form.addEventListener('submit', async (e) => {
        ...
        try {
          const res = await fetch('/api/reviews/reply', { method: 'POST', body: formData });
          const data = await res.json();
      ...
      try {
        ...
        if (data.status === 'success') {
          ...
          document.querySelectorAll('#star-rating-input .star-btn').forEach(s => {
            s.innerHTML = STAR_SVG_EMPTY_INPUT;
          });
          ...
          setTimeout(() => {
            const newCard = document.querySelector('[data-review-id="' + data.review.id + '"]');
            if (newCard) {
